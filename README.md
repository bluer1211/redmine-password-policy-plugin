...
- **防止鍵盤模式**：禁止使用 `1qaz2wsx`、`#EDC$RFV` 等鍵盤位置模式（至少連續 4 字符才觸發）
...
| **防止常見密碼** | 禁止使用常見弱密碼 | ✅ 啟用 |
| **防止連續字符** | 禁止使用連續字符 | ✅ 啟用 |
| **防止鍵盤模式** | 禁止使用鍵盤位置模式（至少連續 4 字符） | ✅ 啟用 |
| **防止重複字符** | 禁止使用重複字符 | ✅ 啟用 |

### 鍵盤模式判定規則（v2.1.8 起）

為了降低誤判，鍵盤位置模式改為 **至少連續 4 字符才觸發**：

- ✅ 會觸發：`qwer`、`1qaz`、`1472`
- ❌ 不會觸發：`qwe`、`asd`、`147`

這樣可以避免正常密碼中偶然出現 3 字符片段時被誤擋。
...
### v2.1.8 (2026-03-07)
- 🔐 調整鍵盤模式判定門檻：改為至少連續 4 字符才觸發
- ✅ 降低 3 字符片段（如 `qwe`、`147`）造成的誤判
- 🧪 補齊對應測試與驗收案例

### v2.1.7 (2026-03-07)
- 🧪 CI 分流：stub 與 integration-only 分開執行
- 🔍 保留 CodeQL 安全掃描

### v2.1.6 (2026-03-07)
- 🐛 修正布林設定值 `'0'` 在 Ruby truthy 下的誤判問題
- ✅ 修復停用規則時仍被套用的風險

### v2.1.5 (2025-09-17)
- 🛡️ **完整鍵盤直列模式檢測**: 100% 攔截率
- 🔧 新增反向直列模式: zaq1, xsw2, cde3, vfr4, bgt5, nhy6, mju7, ki8, lo9, p0
- ✅ 新增部分直列模式: 1qa, qaz, 2ws, wsx, 3ed, edc, 4rf, rfv, 5tg, tgb, 6yh, yhn, 7uj, ujm, 8i, ik, 9o, ol
- 📊 新增短直列模式: 1q, qa, az, 2w, ws, sx, 3e, ed, dc, 4r, rf, fv, 5t, tg, gb, 6y, yh, hn, 7u, uj, jm, 8i, ik, 9o, ol, 0p, p0
- 🎯 攔截所有從1到0的鍵盤直列輸入懶人密碼
- 🧪 通過55個直列模式測試，攔截率100%

### v2.1.4 (2025-09-17)
- 🔧 添加跨行鍵盤模式檢測
- ✅ 修復 %TGB^YHNik 等複雜模式檢測
- 🛡️ 增強不含數字的跨行模式
